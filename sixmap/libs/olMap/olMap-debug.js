...
//          maxResolution: 45,
    ...
    Map.count = 0;
    Map.animating = false;
    Map.startTracking = function (iMonitoring, iCoords) {
        ...
        for (var j = 0; j < iCoords.length; j++) {
            ...
            feature.setId(j);
            iMonitoring.locationLayer.getSource().addFeature(feature);
        }
        iMonitoring.locationLayer.setStyle(null);
        var ifeatures = iMonitoring.locationLayer.getSource().getFeatures();
        //设置显示车辆的初始位置
        ...
        var timer = setInterval(function () {
            ...
            if (Map.count > iCoords.length) {
                ...
                var ifeature = iMonitoring.locationLayer.getSource().getFeatureById(Map.count);
                ifeature.setStyle(new ol.style.Style({
                    image: new ol.style.Icon({
                        offset: [0, 0],
                        opacity: 1.0,
                        rotateWithView: true,
